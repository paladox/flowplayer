   ...
   return self = {
      ...
      load: function(video) {
         ...
         if (conf.splash && !api) {
           ...
           common.prop(api, 'controls', false);
         ...
         if (conf.live || video.live) {
           bean.one(api, 'timeupdate', function() { api.currentTime = 0; }); // Workaround wrong offset with regard to seekable
         }
      },
          ...
          if (!e.target || !ClassList(e.target).contains('fp-engine')) return;
            ...
            switch (flow) {
               ...
               case "ready":
                  ...
                  if (player.live && !arg.duration && api.seekable.length) {
                    arg.duration = api.seekable.end(null) - api.seekable.start(null);
                    arg.liveOffset = api.seekable.start(null);
                  }
               ...
               case "progress": case "seek":
                  ...
                  if (player.live && api.seekable.length) {
                    player.video.liveOffset = api.seekable.start(null);
                    player.video.duration = api.seekable.end(null) - api.seekable.start(null);
                  }
